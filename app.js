...
function opsUserFilterBar(withAuditFilters) {
  ...
  return `
    <div class="res-filters ops-user-filters">
      ...
      <label class="res-select res-select-sm ops-user-period">
        <select aria-label="终端类型" data-ops-user-platform>
          <option value="all" ${opsState.userPlatform === "all" ? "selected" : ""}>全部终端</option>
          <option value="desktop" ${opsState.userPlatform === "desktop" ? "selected" : ""}>桌面端</option>
          <option value="web" ${opsState.userPlatform === "web" ? "selected" : ""}>网页端</option>
        </select>
        ${RES_ICONS.chevron}
      </label>
      ${auditFields}
...
document.addEventListener("change", (event) => {
  ...
  const opsUserPlatform = event.target.closest("[data-ops-user-platform]");
  if (opsUserPlatform) {
